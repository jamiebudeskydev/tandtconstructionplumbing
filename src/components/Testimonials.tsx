...
export default function Testimonials() {
  return (
    <section
      id="testimonials"
      className="relative py-24 md:py-32 bg-[#141B2D] overflow-hidden"
    >
      {/* Background effects */}
      <div className="absolute inset-0 stripe-pattern opacity-20" />
      <div className="absolute -top-40 left-1/2 -translate-x-1/2 h-[500px] w-[800px] rounded-full bg-[#C84B31]/6 blur-[120px]" />
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto">
          ...
          {/* Aggregate rating */}
          <div className="mt-8 inline-flex items-center gap-4 rounded-full border border-[#C84B31]/30 bg-[#C84B31]/10 px-6 py-3">
            <div className="flex gap-1">
              {[...Array(5)].map((_, i) => (
                <svg key={i} className="h-5 w-5 text-[#C84B31]" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
              ))}
            </div>
            <span className="font-[var(--font-body)] text-sm font-medium text-[#E07A5F]">
        ...
        <div className="mt-16 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {testimonials.map((t, i) => (
            <div
              key={i}
              className="group rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-8 transition-all hover:border-[#C84B31]/30 hover:bg-white/8"
            >
              {/* Stars */}
              <div className="flex gap-1">
                {[...Array(t.rating)].map((_, j) => (
                  <svg key={j} className="h-4 w-4 text-[#C84B31]" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                  </svg>
                ))}
              </div>
              ...
              {/* Quote */}
              <blockquote className="mt-4 font-[var(--font-body)] text-sm leading-relaxed text-[#B8C0D2]">
                &ldquo;{t.quote}&rdquo;
              </blockquote>
              ...
              <div className="mt-6 flex items-center justify-between">
                ...
                <span className="rounded-full bg-white/10 px-3 py-1 font-[var(--font-body)] text-xs text-[#8E9AB3]">
                  {t.source}
                </span>
              </div>
            </div>
